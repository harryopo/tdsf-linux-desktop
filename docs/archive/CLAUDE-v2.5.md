...
### B1 · 单文件行数（质量优先 · 2026-07-23 用户放宽）

> **v2.5 放宽**：用户规定「保证质量」是核心目标，单文件行数不再是强制红线。文件是否拆分取决于**质量需要**，而非单纯行数。
>
> 行数过多本身不是缺陷；只有当大文件导致以下质量问题时才拆分：
> - 职责混杂，单函数/单组件承担多个不相关功能
> - 重复代码难以维护
> - 测试困难，覆盖率上不去
> - 编译/加载性能明显下降
>
> **原则**：不为了拆分而拆分。拆分是为了提升可读性、可测试性、可维护性。

- 不强制 ≤ 500 行
- 当文件超过 800 行且出现上述质量问题时，评估拆分
- 大文件必须结构清晰：按职责分 region/section，函数/组件内聚
- 如需拆分，按功能职责拆为多个模块；不要为凑行数而过度拆分
...
| **B 约束** | B1 单文件行数 | 质量优先，按需拆分 | 不强制 ≤ 500 行 |
...
*CLAUDE.md v2.5 · 2026-07-23 · B1 单文件行数放宽为质量优先按需拆分*
