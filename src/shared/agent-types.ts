...
// ----------------------------------------------------------------------------
// 设计原则：
// - SSOT 兼容：本文件中的类型与 src/main/core/agent/credibility/calibration/types.ts
//   保持完全一致（结构、字段、语义均相同），修改时请同步两处。
// - 不依赖 main 进程模块：preload/renderer 可安全导入。
// - 论文支撑：Guo et al. 2017, "On Calibration of Modern Neural Networks", ICML
//   arXiv:1706.04599 §3.1-3.2
// ============================================================================

/**
 * Provider 标识（用于按 LLM 类型分类校准）
 *
 * 常见取值：
...
export interface CalibrationSample {
   ...
   *
   * 论文依据（Zhao 2026, arXiv:2603.18940）：
   * - 熵轨迹**形状**（是否单调递减）比标量总熵更具预测力
   * - 单调链 68.8% 准确率 vs 非单调链 46.8%
   * - 详见 main/core/agent/credibility/mass-functions/cot-trace-signal.ts
   *
   * 注意：
   * - 数组合法值 ∈ [0, 1]；非法值（NaN/Infinity/<0/>1）由 analyzeCotEntropyTrajectory 兜底
   * - 数组通过 IPC structured clone 传输
   * - 不会影响 Temperature Scaling 网格搜索（仍按 reportedConfidence 拟合 T）
   * - 仅在 ai-param 证据融合时作为辅助置信度信号参与
   */
  cotEntropyTrajectory?: number[]
}
