/**
 * monitor-sample — MonitorPage 测试夹具
 *
 * 用途：
 * - 仅在测试用例 / Storybook / 开发预览（import.meta.env.DEV）中使用
 * - 运行时（MonitorPage / AlertTable / Charts / ProcessTable）不再加载此 mock 数据
 *   （spec REMOVED Requirements：mock 数据仅保留在测试用例中，
 *    IPC 不可用时改用 EmptyState / ErrorState）
 *
 * 数据全部为虚构演示值（1:1 来自 monitor.html），不涉及真实生产数据。
 *
