...
// scripts/import-project-guides.ts
...
var DatabaseManager = class _DatabaseManager {
   ...
   */
  createMockStatement(sql) {
    const trimmedSql = sql.trim().toUpperCase();
    const isWriteOperation = trimmedSql.startsWith("INSERT") || trimmedSql.startsWith("UPDATE") || trimmedSql.startsWith("DELETE") || trimmedSql.startsWith("CREATE") || trimmedSql.startsWith("DROP") || trimmedSql.startsWith("ALTER") || trimmedSql.startsWith("REPLACE");
    return {
      run: (...params) => {
        void params;
        if (isWriteOperation) {
   ...
   */
  migrateDecisionCardsTable() {
    try {
      if (!this.db) return;
      const columns = this.db.prepare("PRAGMA table_info(decision_cards)").all();
  ...
  backfillSearchTables() {
    ...
    try {
      ...
      const dftsCount = this.db.prepare("SELECT COUNT(*) AS c FROM decision_fts").get();
      const cardsCount = this.db.prepare("SELECT COUNT(*) AS c FROM decision_cards").get();
   ...
   */
  prepareDecisionFtsSearch(query, limit = 50) {
    if (!this.dbAvailable || !this.db) {
      return [];
    }
    try {
...
// scripts/import-project-guides.ts
var GUIDES_DIR = (0, import_node_path.join)(process.cwd(), "..", "knowledge", "courses", "\u9879\u76EE\u6307\u5BFC\u6587\u6863");
var SOURCE_NAME = "\u6DF1\u5733\u4FE1\u606F\u804C\u4E1A\u6280\u672F\u5927\u5B66 Linux \u6559\u5B66\u9879\u76EE";
var SOURCE_LICENSE = "Educational Use";
var SOURCE_KIND = "offline-dump";
var CATEGORY_MAP = {
  // 大一上学期
...
function makeTutorialId(relativePath) {
  return `tut-local-${(0, import_node_crypto.createHash)("sha256").update(relativePath).digest("hex").slice(0, 16)}`;
}
function parseTitle(fileName, content) {
  const h1Match = content.match(/^#\s+(.+)$/m);
  if (h1Match) {
    return h1Match[1].trim().replace(/^\S+\s+/, "");
  }
  return fileName.replace(/^(.+?)[_:：]/, "").replace(/\.md$/i, "").replace(/实施报告\d*[：:]?/, "").trim();
}
function parseSummary(content, title) {
  const demandMatch = content.match(/###\s+项目需求[\s\S]*?(?=###|##\s|$)/);
  if (demandMatch) {
    const lines = demandMatch[0].split("\n").filter((l) => l.trim().match(/^\d+\./)).map((l) => l.replace(/^\s*\d+\.\s*/, "").trim());
    if (lines.length > 0) {
...
function scanMdFiles(dir) {
  ...
  for (const entry of (0, import_node_fs.readdirSync)(dir)) {
    ...
    const st = (0, import_node_fs.statSync)(fullPath);
    if (st.isDirectory()) {
...
function parseGuideFile(filePath, projectRoot) {
  const relPath = (0, import_node_path.relative)(projectRoot, filePath);
  const content = (0, import_node_fs.readFileSync)(filePath, "utf-8");
  if (!content.trim()) {
    console.warn(`[import] \u8DF3\u8FC7\u7A7A\u6587\u4EF6: ${relPath}`);
    return null;
  }
  const fileName = filePath.split(import_node_path.sep).pop() || "";
...
async function main() {
  const dbPath = (0, import_node_path.join)(import_electron2.app.getPath("userData"), "tdsf.db");
  ...
  for (const filePath of mdFiles) {
    try {
      ...
      console.error(`[import] \u89E3\u6790\u5931\u8D25: ${filePath}`, err.message);
    }
  }
