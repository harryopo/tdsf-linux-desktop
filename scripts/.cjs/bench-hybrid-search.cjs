...
// scripts/bench-hybrid-search.ts
...
function displayWidth(s) {
  ...
  for (const ch of s) {
    ...
    if (code >= 19968 && code <= 40959 || code >= 12288 && code <= 12543 || code >= 65280 && code <= 65519 || code >= 13312 && code <= 19903) {
      w += 2;
    } else {
      w += 1;
    }
...
async function timeAsync(fn) {
  ...
  return { result, ms: import_node_perf_hooks.performance.now() - start };
}
async function main() {
  ...
  if (!embeddingModelLoaded) {
    ...
    try {
      ...
      console.error("[bench] \u6A21\u578B\u52A0\u8F7D\u5931\u8D25:", err.message);
  ...
  try {
    ...
    console.warn("[bench] \u9884\u70ED\u5931\u8D25\uFF08\u7EE7\u7EED\u6D4B\u8BD5\uFF09:", err.message);
  }
  console.log("");
  ...
  for (let qi = 0; qi < TEST_QUERIES.length; qi++) {
    ...
    for (let run = 0; run < RUNS_PER_QUERY; run++) {
      try {
        ...
        console.warn(`  [FTS run ${run + 1}] \u5931\u8D25: ${err.message}`);
        ftsTimings.push(0);
        ftsCounts.push(0);
      }
      try {
        ...
        const ms = import_node_perf_hooks.performance.now() - start;
        vecTimings.push(ms);
        vecCounts.push(result.length);
      } catch (err) {
        console.warn(`  [Vec run ${run + 1}] \u5931\u8D25: ${err.message}`);
        vecTimings.push(0);
        vecCounts.push(0);
      }
      try {
        ...
        console.warn(`  [Hybrid run ${run + 1}] \u5931\u8D25: ${err.message}`);
    ...
    console.log(
      `  FTS p50=${fmtMs(ftsStats.p50)}  Vec p50=${fmtMs(vecStats.p50)}  Hybrid p50=${fmtMs(hybridStats.p50)}`
    );
    allResults.push({
      query,
      fts: ftsStats,
  ...
  const COL_NUM = 10;
  const header = `| ${padRight("\u67E5\u8BE2", COL_QUERY)} | ${padRight("FTS p50", COL_NUM)} | ${padRight("FTS p95", COL_NUM)} | ${padRight("Vec p50", COL_NUM)} | ${padRight("Vec p95", COL_NUM)} | ${padRight("Hybrid p50", COL_NUM)} | ${padRight("Hybrid p95", COL_NUM)} |`;
  const separator = `| ${"-".repeat(COL_QUERY)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} |`;
  console.log(header);
  console.log(separator);
  for (const r of allResults) {
    console.log(
      `| ${padRight(r.query, COL_QUERY)} | ${padRight(fmtMs(r.fts.p50), COL_NUM)} | ${padRight(fmtMs(r.fts.p95), COL_NUM)} | ${padRight(fmtMs(r.vec.p50), COL_NUM)} | ${padRight(fmtMs(r.vec.p95), COL_NUM)} | ${padRight(fmtMs(r.hybrid.p50), COL_NUM)} | ${padRight(fmtMs(r.hybrid.p95), COL_NUM)} |`
    );
  }
  ...
  console.log(
    `| ${padRight("\u67E5\u8BE2", COL_QUERY)} | ${padRight("FTS avg", COL_NUM)} | ${padRight("Vec avg", COL_NUM)} | ${padRight("Hybrid avg", COL_NUM)} |`
  );
  console.log(
    `| ${"-".repeat(COL_QUERY)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} | ${"-".repeat(COL_NUM)} |`
  );
  for (const r of allResults) {
    ...
    console.log(
      `| ${padRight(r.query, COL_QUERY)} | ${padRight(ftsAvg.toFixed(1), COL_NUM)} | ${padRight(vecAvg.toFixed(1), COL_NUM)} | ${padRight(hybridAvg.toFixed(1), COL_NUM)} |`
    );
  }
  console.log("");
